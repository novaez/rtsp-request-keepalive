#
# RTSP-Lite-Keepalive.pm 0.1
#   Base on RTSP::Lite on 
#   http://www.kosho.org/tools/rtsp-lite/
#

package RTSP::Lite::Keepalive;
...
$SIG{PIPE} = sub { die "Connection has been closed by server.\n"; };
...
sub initialize
{
    ...
    $self->{DEBUG} = 0;
    $self->{cseq} = 0;
    $self->{user_agent} = "RTSP::Lite::Keepalive 0.1";
    $self->reset;
}
...
sub open
{
    ...
    # If there is a connection opened, use it directly.
    if(tell(FH) != -1) {
        $self->reset;
        return 1;
    } else {
...
{
    my ($self) = @_;
    $self->rtsp_write(*FH, "RTSP/1.0 200 OK$CRLF");
    $self->{cseq}++;
    $self->rtsp_write(*FH, "cseq: ".$self->{cseq}."$CRLF");
}
...
#  }
    ...
    my $result = $self->listen($self->{timeout}, $data_callback, $cbargs);
...
sub listen
{
    my ($self, $timeout, $data_callback, $cbargs) = @_;
...
{
    my $self = shift;
    my ($fh, $headmode, $chunkmode, $chunksize, $timeout) = @_;
	...
	my $bytes_to_read = $chunkmode eq "chunk" ?
	    ...
	    $BLOCKSIZE;
	$res = $self->rtsp_readbytes($fh, $timeout, $bytes_to_read);
    } else {
	$res = $self->rtsp_readline($fh, $timeout);
    }
...
RTSP::Lite::Keepalive - Lightweight RTSP implementation with keepalive

=head1 SYNOPSIS

  use RTSP::Lite::Keepalive;
  $rtsp = new RTSP::Lite::Keepalive;
...
RTSP::Lite::Keepalive is a stand-alone lightweight RTSP/1.0 module 
for Perl. It is based on RTSP::Lite, which is based on Roy Hooper's 
HTTP::Lite (RTSP protocol is very similar to HTTP protocol. I simply
modified it to support RTSP).
...
Set the agent name (Default is "RTSP::Lite::Keepalive 0.1"). 
...
You must call this prior to re-using an RTSP::Lite::Keepalive file handle,
